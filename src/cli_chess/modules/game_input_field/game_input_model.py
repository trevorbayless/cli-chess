...
class GameInputModel:
    def __init__(self, board_model: BoardModel) -> None:
        ...
        self.e_game_input_model_updated = self._event_manager.create_event()

    def update(self, *args, **kwargs) -> None: # noqa
        """Update method used for subscriptions this model has to others"""
        # if EventTopics.GAME_END in args:
        #     self.clear_premove()
        pass

    def get_move_input_hint_text(self) -> str:
        """Returns the move input hint text. This is the resolved SAN
           when input matches exactly one legal move (for the hint line)
        """
    ...
    def _notify_game_input_model_updated(self) -> None:
        """Notifies listeners of game input model updates"""
        self.e__model_updated.notify()
